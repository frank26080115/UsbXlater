                               ...
                               USB_SETUP_REQ *req)
{
  ...
  case USB_DESC_TYPE_DEVICE:
    pbuf = pdev->dev.cb->GetDeviceDescriptor(pdev->cfg.speed, &len);
  ...
  case USB_DESC_TYPE_CONFIGURATION:
      pbuf   = (uint8_t *)pdev->dev.cb->GetConfigDescriptor(pdev->cfg.speed, &len);
#ifdef USB_OTG_HS_CORE
    if((pdev->cfg.speed == USB_OTG_SPEED_FULL )&&
       (pdev->cfg.phy_itface  == USB_OTG_ULPI_PHY))
    {
      pbuf   = (uint8_t *)pdev->dev.cb->GetOtherConfigDescriptor(pdev->cfg.speed, &len);
    }
#endif
  ...
  case USB_DESC_TYPE_STRING:
    ...
    case USBD_IDX_LANGID_STR:
     pbuf = pdev->dev.cb->GetLangIDStrDescriptor(pdev->cfg.speed, &len);
      break;

    case USBD_IDX_MFC_STR:
      pbuf = pdev->dev.cb->GetManufacturerStrDescriptor(pdev->cfg.speed, &len);
      break;

    case USBD_IDX_PRODUCT_STR:
      pbuf = pdev->dev.cb->GetProductStrDescriptor(pdev->cfg.speed, &len);
      break;

    case USBD_IDX_SERIAL_STR:
      pbuf = pdev->dev.cb->GetSerialStrDescriptor(pdev->cfg.speed, &len);
      break;

    case USBD_IDX_CONFIG_STR:
      pbuf = pdev->dev.cb->GetConfigurationStrDescriptor(pdev->cfg.speed, &len);
      break;

    case USBD_IDX_INTERFACE_STR:
      pbuf = pdev->dev.cb->GetInterfaceStrDescriptor(intf, pdev->cfg.speed, &len); // TODO: add intf parameter
      break;
...
void USBD_ParseSetupRequest( USB_OTG_CORE_HANDLE  *pdev,
                            USB_SETUP_REQ *req)
{
  req->bmRequest     = pdev->dev.setup_packet[0]; // *(uint8_t *)  (pdev->dev.setup_packet);
  req->bRequest      = pdev->dev.setup_packet[1]; // *(uint8_t *)  (pdev->dev.setup_packet +  1);
  req->wValue        = pdev->dev.setup_packet[3]; // SWAPBYTE      (pdev->dev.setup_packet +  2);
  req->wValue      <<= 8;
  req->wValue       += pdev->dev.setup_packet[2];
  req->wIndex        = pdev->dev.setup_packet[5]; // SWAPBYTE      (pdev->dev.setup_packet +  2);
  req->wIndex      <<= 8;
  req->wIndex       += pdev->dev.setup_packet[4];
  req->wLength       = pdev->dev.setup_packet[7]; // SWAPBYTE      (pdev->dev.setup_packet +  2);
  req->wLength     <<= 8;
  req->wLength      += pdev->dev.setup_packet[6];

...
//#include <usbh_dev/usbh_dev_manager.h> // TODO: make
...
* @param  pcore->host.Rx_Buffer: Receive Buffer address
                             ...
                             uint8_t length)
{
  ...
  if((status = USBH_GetDescriptor(pcore,
                                  pdev,
                                  ...
                                  length)) == USBH_OK)
  {
    ...
    USBH_ParseDevDesc(&pdev->device_prop.Dev_Desc, pcore->host.Rx_Buffer, length);
  }
  return status;
}
...
* @retval Status
*/
USBH_Status USBH_Get_CfgDesc(USB_OTG_CORE_HANDLE *pcore,
                             USBH_DEV            *pdev,
                             uint16_t            length)

{
                                  ...
                                  length)) == USBH_OK)
  {
    ...
    /* Commands successfully sent and Response Received  */
    USBH_ParseCfgDesc (&pdev->device_prop.Cfg_Desc,
                       pdev->device_prop.Itf_Desc,
                       pdev->device_prop.Ep_Desc,
                       pcore->host.Rx_Buffer,
                       length);
...
* @retval Status
*/
USBH_Status USBH_Get_StringDesc(USB_OTG_CORE_HANDLE *pcore,
                                USBH_DEV *pdev,
                                ...
                                uint16_t length)
{
  ...
  {
    /* Commands successfully sent and Response Received  */
    USBH_ParseStringDesc(pcore->host.Rx_Buffer,buff, length);
  }
  return status;
}
...
* @param  intf: interface number if required
* @param  buff: Buffer to store the descriptor
* @param  length: Length of the descriptor
* @retval Status
*/
USBH_Status USBH_GetDescriptor(USB_OTG_CORE_HANDLE *pcore,
                               USBH_DEV           *pdev,
                               uint8_t  req_type,
                               uint16_t value_idx,
                               uint16_t intf,
                               ...
                               uint16_t length )
{
  ...
  pdev->Control.setup.b.wValue.w = value_idx;

  if ((value_idx & 0xff00) == USB_DESC_STRING)
  {
    pdev->Control.setup.b.wIndex.w = 0x0409;
  }
  else
  {
  ...
  return USBH_CtlReq(pcore, pdev, buff , length );
}

/**
* @brief  USBH_GetDescriptor_Blocking
*         Issues Descriptor command to the device. Once the response received,
*         it parses the descriptor and updates the status.
* @param  pcore: Selected Core Peripheral
* @param  req_type: Descriptor type
...
* @retval Status
*/
USBH_Status USBH_SetAddress(USB_OTG_CORE_HANDLE *pcore,
                            USBH_DEV *pdev,
                            uint8_t DeviceAddress)
{
  pdev->Control.setup.b.bmRequestType = USB_H2D | USB_REQ_RECIPIENT_DEVICE | \
    USB_REQ_TYPE_STANDARD;

  pdev->Control.setup.b.bRequest = USB_REQ_SET_ADDRESS;

	pdev->Control.setup.b.wValue.w = DeviceAddress;
	pdev->Control.setup.b.wIndex.w = 0;
...
* @retval Status
*/
USBH_Status USBH_SetCfg(USB_OTG_CORE_HANDLE *pcore,
                        USBH_DEV *pdev,
                        uint16_t cfg_idx)
{
  ...
  pdev->Control.setup.b.bmRequestType = USB_H2D | USB_REQ_RECIPIENT_DEVICE |\
    USB_REQ_TYPE_STANDARD;
  pdev->Control.setup.b.bRequest = USB_REQ_SET_CONFIGURATION;
  pdev->Control.setup.b.wValue.w = cfg_idx;
  pdev->Control.setup.b.wIndex.w = 0;
  pdev->Control.setup.b.wLength.w = 0;
                        ...
                        uint8_t ep_num, uint8_t altSetting)
{
  ...
  pdev->Control.setup.b.bRequest = USB_REQ_SET_INTERFACE;
  pdev->Control.setup.b.wValue.w = altSetting;
                             ...
                             int8_t hc_num)
{
  ...
  pdev->Control.setup.b.bRequest = USB_REQ_CLEAR_FEATURE;
  pdev->Control.setup.b.wValue.w = FEATURE_SELECTOR_ENDPOINT;
  pdev->Control.setup.b.wIndex.w = ep_num;
  pdev->Control.setup.b.wLength.w = 0;

...
static uint16_t ubcsp_calc_crc (uint8_t ch, uint16_t crc);
static uint16_t ubcsp_crc_reverse (uint16_t);
...
static const uint8_t ubcsp_le_buffer[4][4] =
	{
		0xDA, 0xDC, 0xED, 0xED,
		0xAC, 0xAF, 0xEF, 0xEE,
		0xAD, 0xEF, 0xAC, 0xED,
...
#if UBCSP_CRC
static const uint8_t ubcsp_send_le_header[4] = 
	{
		0x40, 0x41, 0x00, 0x7E
	};
#else
...
static uint8_t ubcsp_send_ack_header[4] = 
	{
		0x00, 0x00, 0x00, 0x00
	};
...
static uint8_t next_le_packet[16] =
	{
...
static uint8_t ubcsp_send_header[4];
static uint8_t ubcsp_send_crc[2];

/* This is where the receive header is stored before the payload arrives */

static uint8_t ubcsp_receive_header[4];
...
#if SHOW_LE_STATES
	dbg_printf(DBGMODE_DEBUG, "Hello Link Uninitialized\n");
#endif
...
static uint16_t ubcsp_crc_reverse (uint16_t crc)
{
	int32_t
		b,
		rev;
...
static void ubcsp_put_slip_uart (uint8_t ch)
{
...
static uint32_t ubcsp_which_le_payload (const uint8_t *payload)
{
	static int32_t
		octet,
		loop;
...
static uint8_t ubcsp_recevied_packet (void)
{
	static uint8_t
		receive_crc,
		receive_seq,
	...
	{
		dbg_printf(DBGMODE_DEBUG, " : %10d         Recv SEQ: %d ACK %d\n",
			ubcsp_get_ticks () % 100000,
			receive_seq,
			receive_ack);
	}
	else if (ubcsp_config.receive_packet->channel != 1)
	{
		dbg_printf(DBGMODE_DEBUG, " : %10d          Recv        ACK %d\n",
			ubcsp_get_ticks () % 100000,
			receive_ack);
	}
#endif
	...
	if (ubcsp_config.receive_packet->channel == 1)
	{
		...
		switch (ubcsp_which_le_payload (ubcsp_config.receive_packet->payload))
		{
...
#if SHOW_LE_STATES
				dbg_printf(DBGMODE_DEBUG, "Recv SYNC\n");
#endif
			...
			case 1:
			{
...
#if SHOW_LE_STATES
				dbg_printf(DBGMODE_DEBUG, "Recv SYNC RESP\n");
#endif
				...
				{
#if SHOW_LE_STATES
					dbg_printf(DBGMODE_DEBUG, "Link Initialized\n");
#endif
...
#if SHOW_LE_STATES
				dbg_printf(DBGMODE_DEBUG, "Recv CONF\n");
#endif
			...
			case 3:
			{
...
#if SHOW_LE_STATES
				dbg_printf(DBGMODE_DEBUG, "Recv CONF RESP\n");
#endif
				...
				{
#if SHOW_LE_STATES
					dbg_printf(DBGMODE_DEBUG, "Link Active\n");
#endif
...
static void ubcsp_setup_packet (uint8_t *part1, uint8_t calc, uint8_t *part2, uint16_t len2)
{
...
static uint8_t ubcsp_sent_packet (void)
{
	...
	if (!ubcsp_config.delay)
	{
		...
		if (ubcsp_config.send_size)
		{
			...
			if (ubcsp_config.send_slip_escape)
			{
				...
				hcicereal_tx (ubcsp_config.send_slip_escape);
			...
			if ((!ubcsp_config.send_slip_escape) && ((ubcsp_config.send_size = ubcsp_config.send_size - 1) == 0))
			{
				...
				if (!ubcsp_config.send_ptr)
				{
					if (ubcsp_config.need_send_crc)
					{
						...
						ubcsp_send_crc[0] = (uint8_t) (ubcsp_config.send_crc >> 8);
						ubcsp_send_crc[1] = (uint8_t) ubcsp_config.send_crc;
		...
		else if (ubcsp_config.link_establishment_packet == ubcsp_le_none)
		{
			...
			if (ubcsp_config.link_establishment_resp & 2)
			{
				...
				ubcsp_setup_packet ((uint8_t*) ubcsp_send_le_header, 0, (uint8_t*) ubcsp_le_buffer[ubcsp_le_conf_resp], 4);
			...
			else if (ubcsp_config.send_packet)
			{
				...
				ubcsp_setup_packet ((uint8_t*) ubcsp_send_header, 1, ubcsp_config.send_packet->payload, ubcsp_config.send_packet->length);
...
#if SHOW_PACKET_ERRORS
				dbg_printf(DBGMODE_DEBUG, " : %10d Send %d Ack %d\n",
					ubcsp_get_ticks () % 100000,
					ubcsp_config.sequence_number,
					ubcsp_config.ack_number);
#endif
...
#if SHOW_PACKET_ERRORS
				dbg_printf(DBGMODE_DEBUG, " : %10d Send ACK %d\n",
					ubcsp_get_ticks () % 100000,
					ubcsp_config.ack_number);
#endif
		...
		{
#if SHOW_PACKET_ERRORS
//			dbg_printf(DBGMODE_DEBUG, " : %10d Send LE %d\n",
//				ubcsp_get_ticks () % 100000,
//				ubcsp_config.link_establishment_packet);
			...
			ubcsp_setup_packet ((uint8_t*) ubcsp_send_le_header, 0, (uint8_t*) ubcsp_le_buffer[ubcsp_config.link_establishment_packet], 4);
	...
	while ((ubcsp_config.receive_packet) && (hcicereal_get (&value)))
	{

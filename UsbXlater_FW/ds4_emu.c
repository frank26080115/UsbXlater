...
{
	SDP_PDU_ID_t pdu_id;
	uint16_t* conn_handle_ptr = &packet[-COMPLETE_L2CAP_HEADER];
		...
		case HCI_EVENT_PACKET:
			switch (packet[0])
			{
				...
				case L2CAP_EVENT_CHANNEL_OPENED:
					...
					{
						dbg_printf(DBGMODE_ERR, "L2CAP_EVENT_CHANNEL_OPENED with bad status 0x%02X\r\n", packet[2]);
						ret = 1;
					}
					break;
			...
			break;
		case L2CAP_DATA_PACKET:
			if (channel == bt2ps_hidintr_channel && bt2ds_hidintr_channel != 0) {
				ptr_ringbuffer_push(&ps2ds_hidintr_queue, packet, size);
				ret = 1;
			}
			else if (channel == bt2ps_hidctrl_channel && bt2ds_hidctrl_channel != 0) {
				ptr_ringbuffer_push(&ps2ds_hidctrl_queue, packet, size);
				ret = 1;
			}
			else if (channel == bt2ds_hidintr_channel && bt2ps_hidintr_channel != 0) {
				if (packet[1] == 0x11 || packet[1] == 0x12 || packet[1] == 0x13) {
					// these reports contain button and stick data, but the longer ones contain audio
			...
			else if (channel == bt2ds_hidintr_channel && bt2ps_hidctrl_channel != 0) {
...
void ds4emu_reroute_channel(ptr_ringbuffer_t* queue, uint16_t local_cid)
{
	if (ptr_ringbuffer_isempty(queue) == 0)
	{
		if (local_cid == 0) {
			...
			int length = ptr_ringbuffer_pop(queue, l2cap_get_outgoing_buffer());
			if (length > 0)
			{
...
int btproxy_db_get_link_key(bd_addr_t *bd_addr, link_key_t *link_key, link_key_type_t * type) {
	*type = UNAUTHENTICATED_COMBINATION_KEY_GENERATED_FROM_P192;
	if (memcmp(bd_addr, ps4_bdaddr, BD_ADDR_LEN) == 0) {
